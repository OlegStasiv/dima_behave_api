...
def set_id(element_id):
    with open('./features/conf.yaml') as f:
        doc = yaml.load(f)

    doc['id'] = element_id

    with open('./features/conf.yaml', 'w') as f:
        yaml.dump(doc, f)
...
def get_request(context, request_verb, url_path_segment):
    ...
    if request_verb.lower() == 'delete' or request_verb.lower() == 'put' or request_verb.lower() == 'patch':
        url = context.base_url + '/' + url_path_segment + '/' + str(context.my_id)
...
@step('I make a {request_verb} request to "{url_path_segment}" by id')
def get_request(context, request_verb, url_path_segment):
    if not hasattr(context, 'verify_ssl'):
        context.verify_ssl = True
...
def request_with_parameters(context, request_verb, url_path_segment):
    ...
    if context.r.status_code == 201:
        set_id(context.r.json()['id'])

    log_full(context.r)

    return context.r

@step('I make a {request_verb} request to "{url_path_segment}" with parameters with "{image}" file')
def request_with_parameters(context, request_verb, url_path_segment, image):
    if not hasattr(context, 'verify_ssl'):
        context.verify_ssl = True
    ...
    context.r = getattr(requests, request_verb.lower())(url, files=files, data=params)
